...
<?php
    class DV_Contact_Type{
        ...
        public static function listen(){
			...
			if (!isset($_POST["id"]) || !isset($_POST["own"]) || !isset($_POST["type"])) {
				return rest_ensure_response( 
					array(
						"status" => "unknown",
						"message" => "Please contact your administrator. Request unknown!",
            ...
            if (empty($_POST["id"]) || empty($_POST["own"]) || !isset($_POST["type"])) {
				return rest_ensure_response( 
					array(
						"status" => "failed",
            ...
            if ($_POST['own'] == 'user') {
                ...
                WHERE dc.`status` = 1 AND dc.types = '$type' AND dc.wpid = $id ", OBJECT);
            ...
            if ($_POST['own'] == 'store') {
                ...
                FROM
                    $table_contact dc
                    INNER JOIN $table_revs dr ON dr.ID = dc.revs
                WHERE dc.`status` = 1 AND dc.types = '$type' AND dc.stid = $id ", OBJECT);
            }
